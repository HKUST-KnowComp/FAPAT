...
if __name__ == "__main__":
    ...
    def process_sequences(ids, dates, sequences, attributes):
        out_ids = []
        out_dates = []
        out_sequences = []
        ...
        for idx, _id in enumerate(ids):
            ...
            for i in range(1, len(seq)):
                ...
                for k, v in attributes.items():
                    ...
                    out_attr_labels[k].append(v[idx][i])
    ...
    train = process_sequences(train_ids, train_dates, train_sequences, train_attributes)
    valid = process_sequences(valid_ids, valid_dates, valid_sequences, valid_attributes)
    test = process_sequences(test_ids, test_dates, test_sequences, test_attributes)

    print("train - valid - test:", len(train[0]), "-", len(valid[0]), "-", len(test[0]))

    keys = ["ids", "dates", "input", "target"]
    pickle.dump(dict(zip(keys, train)), open("diginetica/train.pkl", "wb"))
    pickle.dump(dict(zip(keys, valid)), open("diginetica/valid.pkl", "wb"))
    pickle.dump(dict(zip(keys, test)), open("diginetica/test.pkl", "wb"))
